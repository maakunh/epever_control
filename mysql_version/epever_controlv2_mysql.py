...
if (len(sys.argv) < 1):
	print("Usage: epever_controlvx.py <control number> <line message on:2/off:9>")
	sys.exit(0)
else:
    ctrlNum = sys.argv[1]
...
try:
    for portName in portNames:
        ...
        print("port: " + portName + " val(V): " + str(response))
        strr = str(response)
        ...
        print("port: " + portName + " val(A): " + str(response))
        strr = str(response)
        cls_epever_control_tool.to_numericval_unit(strr)
    ...
    linemsg_msg = "ModbusClient Error (" + portName + ") epever_controlv2_mysql.py"
...
if iCount == 0: #retry failure
    valV = 0.0
    sys.exit(1) #do not control
else:
    valV = valV / iCount    #average of cc output voltage
    d = Decimal(valV)
    valV = d.quantize(Decimal('1e-2'))

d = Decimal(valC)
valC = d.quantize(Decimal('1e-2'))
...
ret = cls_epever_control_db.read_control(ctrlNum)
if ret == lvNormal:
...
if dt_now > Starttime:
    if dt_now < Endtime: #Process Running
        if valV > Vmax:
            result = subprocess.run(py + ' ' + numato_relayread_py + ' ' + numato_portName + ' ' + numato_baudrate + ' ' + Relay1, shell=True,  stdout=PIPE, stderr=PIPE, timeout=5)
            relay_status1 = result.stdout.decode().replace(epever_control_setting.epever_control_stdout_rtncode(),'')
            if relay_status1 == "off":
                ...
                linemsg_msg = 'relay' + Relay1 + ' on <br>control start 1st relay<br>Voltage = '+ str(valV) + '(>' + str(Vmax) + 'V) Current = ' + str(valC) + '<br>'
                    ...
                    result = subprocess.run(py + ' ' + numato_relayread_py + ' ' + numato_portName + ' ' + numato_baudrate + ' ' + Relay2, shell=True,  stdout=PIPE, stderr=PIPE, timeout=5)
                    relay_status2 = result.stdout.decode().replace(epever_control_setting.epever_control_stdout_rtncode(),'')
                    if relay_status2 == "off":
                        ...
                        result = subprocess.run(py + ' ' + numato_relayread_py + ' ' + numato_portName + ' ' + numato_baudrate + ' ' + Relay3, shell=True,  stdout=PIPE, stderr=PIPE, timeout=5)
                        relay_status3 = result.stdout.decode().replace(epever_control_setting.epever_control_stdout_rtncode(),'')
...
#GTI OFF
            ...
            print(py + ' ' + numato_relayread_py + ' ' + numato_portName + ' ' + numato_baudrate + ' ' + Relay3)              
            result = subprocess.run(py + ' ' + numato_relayread_py + ' ' + numato_portName + ' ' + numato_baudrate + ' ' + Relay3, shell=True,  stdout=PIPE, stderr=PIPE, timeout=5)
            print(result.stdout.decode())
            relay_status3 = result.stdout.decode().replace(epever_control_setting.epever_control_stdout_rtncode(),'')
            print("3:" + relay_status3)
            if relay_status3 == "on":
                result = subprocess.run(py + ' ' + numato_relaywrite_py + ' ' + numato_portName + ' ' + numato_baudrate + ' ' + Relay3 + ' off', shell=True)
                ...
                linemsg_msg = 'relay' + Relay3 + ' off <br>dt_time>Startime <br>dt_time>Endtime <br>process end(time over)<br>'
                    ...
                    print(py + ' ' + numato_relayread_py + ' ' + numato_portName + ' ' + numato_baudrate + ' ' + Relay2)
                    result = subprocess.run(py + ' ' + numato_relayread_py + ' ' + numato_portName + ' ' + numato_baudrate + ' ' + Relay2, shell=True,  stdout=PIPE, stderr=PIPE, timeout=5)
                    print(result)
                    relay_status2 = result.stdout.decode().replace(epever_control_setting.epever_control_stdout_rtncode(),'')
                    print("2:" + relay_status2)
                    if relay_status2 == "on":
                       result = subprocess.run(py + ' ' + numato_relaywrite_py + ' ' + numato_portName + ' ' + numato_baudrate + ' ' + Relay2 + ' off', shell=True)
                       ...
                       linemsg_msg = 'relay' + Relay2 + ' off <br>dt_time>Startime <br>dt_time>Endtime <br>process end(time over)<br>'
                        ...
                        print(py + ' ' + numato_relayread_py + ' ' + numato_portName + ' ' + numato_baudrate + ' ' + Relay1)
                        result = subprocess.run(py + ' ' + numato_relayread_py + ' ' + numato_portName + ' ' + numato_baudrate + ' ' + Relay1, shell=True,  stdout=PIPE, stderr=PIPE, timeout=5)
                        print(result)
                        relay_status1 = result.stdout.decode().replace(epever_control_setting.epever_control_stdout_rtncode(),'')
                        print("1:" + relay_status1)
                        if relay_status1 == "on":
                            result = subprocess.run(py + ' ' + numato_relaywrite_py + ' ' + numato_portName + ' ' + numato_baudrate + ' ' + Relay1 + ' off', shell=True)
...
#write database
retv = cls_epever_control_db.write_control_history(lvalue)

print("linemsg_update_flg =" + str(linemsg_update_flg))

if linemsg_update_flg == flgon:
    #request sending LINE message to database
    print(py + ' ' + linemsg_py + ' r ' + ' "' + linemsg_msg + '" "' + linemsg_application + '"')
